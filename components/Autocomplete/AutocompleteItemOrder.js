      ...
      switch (selected.type) {
        ...
        case "G":
          if (selected.showGroup === "Y") {
            ...
            setLoadingItem(true)
            axios
              .post(ServiceUrl.urlOrder + "FindItemOrderByConsistof", reqItem)
              .then((response) => {
  ...
  const handleInputItem = (value, reason) => {
    ...
    if (reason === "input") {
      ...
      if (value !== "") {
        ...
        setLoadingItem(true)
        axios
          .post(ServiceUrl.urlOrder + "GetItemOrderCriteria", reqItem)
          .then((response) => {
  ...
  return (
    <div>
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField {...params} size="small" variant="outlined" InputProps={{
            ...
            endAdornment: (
              <React.Fragment>
                {loadingItem ? <CircularProgress style={{ marginRight: "40px" }} color="primary" size={20} /> : null}
      ...
      <ModalLookUp
        ...
        serviceName="FindItemOrderByConsistof"
